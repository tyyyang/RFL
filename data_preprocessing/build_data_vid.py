...
def process(obj, data_path):
    ...
    x_sclae = config.x_instance_size / x_size
    ...
    roi_patch = roi_patch.resize([config.x_instance_size, config.x_instance_size])

    # shift bbox relative to roi
    roi_model_size = np.array([config.x_instance_size, config.x_instance_size])
    bb_c_on_roi = (roi_model_size - 1) / 2
